...
public class CustomerMySQLDAO implements ICustomerDAO
{
    ...
    private Customer processRow(HashMap<String,Object> row)
    {
        ...
        if(customer == null)
        {
            customer = new Customer(mySQLConnection,(Integer) row.get("id"), (String) row.get("name"), (String) row.get("shortName"));
        ...
        {
            Employee employee = mySQLConnection.employeeMySQLDAO.findById((Integer)childRow.get("employees_id"));
    ...
    {
        if(customersUpdating.contains(customer)) return true; //already updating
    ...
    public boolean deleteCustomer(Customer customer)
    {
        ...
        {
            mySQLConnection.projectMySQLDAO.deleteProject(project);
        }
    ...
    @Override
    public boolean linkEmployee(Customer customer, Employee employee)
    {
        if(customersUpdating.contains(customer)) return true; //already updating
    ...
    @Override
    public boolean unlinkEmployee(Customer customer, Employee employee)
    {
        if(customersUpdating.contains(customer)) return true; //already updating
