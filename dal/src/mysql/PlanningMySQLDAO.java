...
public class PlanningMySQLDAO implements IPlanningDAO
{
    ...
    private Planning processRow(HashMap<String,Object> row)
    {
        ...
        for(Planning currentPlanning : planningInstances)
        {
            if(currentPlanning.getId() == (Integer)row.get("id"))
            {
                ...
                currentPlanning.setProjectTask(mySQLConnection.taskMySQLDAO.findById((Integer)row.get("tasks_id")));
                currentPlanning.setEmployee(mySQLConnection.employeeMySQLDAO.findById((Integer)row.get("employeesid")));
                planning = currentPlanning;
            }
        }
        if(planning == null)
        {
            ProjectTask projectTask = mySQLConnection.taskMySQLDAO.findById((Integer)row.get("tasks_id"));
            Employee employee = mySQLConnection.employeeMySQLDAO.findById((Integer)row.get("employeesid"));
            planning = new Planning(mySQLConnection,(Integer) row.get("id"),
    ...
    @Override
    public boolean insertPlanning(Planning planning)
    {
    ...
    {
        if(planningsUpdating.contains(planning)) return true; //already updating
        QueryResult result = new QueryResult(mySQLConnection,String.format(
    ...
    @Override
    public boolean deletePlanning(Planning planning)
    {
        if(planningsUpdating.contains(planning)) return true; //already updating
